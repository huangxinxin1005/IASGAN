import numpy as np
import pickle
from dataIO.dataPreprocess import refine_gt
from dataIO.dataPreprocess import color_convert
...
class IonoDataManager():
    ...
    def get_train_batch(self, id):
        ...
        y_train[0, :, :, :] = color_convert(y_train[0, :, :, :], [0, 0, 0], [1, 1, 1])
    ...
    def get_test_batch(self, t_id):
        ...
        y_test[0, :, :, :] = color_convert(y_test[0, :, :, :], [0, 0, 0], [1, 1, 1])

        art_test[0, :ori_height, :ori_width, :] = art_y[:, :, :] / 1.0
        art_test[0, :, :, :] = refine_gt(art_test[0, :, :, :])
        art_test[0, :, :, :] = color_convert(art_test[0, :, :, :], [0, 0, 0], [1, 1, 1])
    ...
    def get_test_batch1(self):
        ...
        x_test = np.zeros([len(self.test_data_list), self.pad_height, self.pad_height, self.channel_num])
        y_test = np.zeros([len(self.test_data_list), self.pad_height, self.pad_height, self.class_num])

        for i, data in enumerate(self.test_data_list):
            ori_x_name = self.base_path + data.split(' ')[0]
            ori_y_name = self.base_path + data.split(' ')[1]
            ...
            ori_height = np.shape(ori_x)[0]
            ori_width = np.shape(ori_x)[1]
            x_test[i, :ori_height, :ori_width, 0] = ori_x[:, :, 0] / np.max(ori_x[:, :, 0])
            x_test[i, :ori_height, :ori_width, 1] = ori_x[:, :, 1] / np.max(ori_x[:, :, 1])
            x_test[i, :ori_height, :ori_width, 2] = ori_x[:, :, 2] / np.max(ori_x[:, :, 2])

            y_test[i, :ori_height, :ori_width, :] = ori_y[:, :, :] / 1.0
            y_test[i, :, :, :] = refine_gt(y_test[i, :, :, :])
            y_test[i, :, :, :] = color_convert(y_test[i, :, :, :], [0, 0, 0], [1, 1, 1])
...
if __name__ == '__main__':
    ...
    x_train, y_train, art_train = dataManager.get_test_batch(0)
